...
from fastapi import APIRouter, HTTPException, Query, Request, UploadFile, status # Added Query

app = APIRouter(prefix='/api/conversations/{conversation_id}')


@app.get('/list-files')
# Use Query(...) to correctly receive the path from query parameters
async def list_files(request: Request, conversation_id: str, path: str | None = Query(None)):
